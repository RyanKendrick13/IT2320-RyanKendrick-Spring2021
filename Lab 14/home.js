        ...
        $.ajax(
          {
            ...
            {
              $("#results").text("");
              console.log(data.name,data.id,data.brewery_type); //if only one item, do data.name, data.id and data.city
              var para = $("<p></p>");
              para.text(data.name+" "+data.id+" "+data.brewery_type);
              $("#results").append(para);
